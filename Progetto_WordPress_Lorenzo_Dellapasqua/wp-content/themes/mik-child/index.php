				...
				<header class="page-header">
					<?php if ( is_home() && is_front_page() ) : 
						if ( ! empty( $latest_blog_title ) ) : ?>
							<h1 class="page-title <?php echo $dot ? 'add-separator' : ''; ?>"><?php echo esc_html( $latest_blog_title ); ?></h1>
						<?php endif;
					else: ?>
...
<!-- Altero il loop per poter far comparire nella Home i custom Article tipo monumento-->
<?php
$args = array(
	'posts_per_page' => 3,
	'post_type' => 'monumento',
	...
	<?php
					...
					if ( $myquery->have_posts() ) : 
						/* Start the Loop */
						while ( $myquery->have_posts()) : $myquery->the_post('monumento');
							/*
							 * Include the Post-Format-specific template for the content.
							 * If you want to override this in a child theme, then include a file
							 * called content-___.php (where ___ is the Post Format name) and that will be used instead.
